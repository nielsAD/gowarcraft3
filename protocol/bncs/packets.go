...
// Serialize encodes the struct into its binary form.
func (pkt *AuthAccountChangePassResp) Serialize(buf *protocol.Buffer) error {
	var start = buf.Size()
	if err := pkt.AuthAccountLogonResp.Serialize(buf); err != nil {
		return err
	}
	buf.WriteUInt8At(start+1, PidAuthAccountChange)
	return nil
}
...
// Serialize encodes the struct into its binary form.
func (pkt *AuthAccountChangePassReq) Serialize(buf *protocol.Buffer) error {
	var start = buf.Size()
	if err := pkt.AuthAccountLogonReq.Serialize(buf); err != nil {
		return err
	}
	buf.WriteUInt8At(start+1, PidAuthAccountChange)
	return nil
...
// Serialize encodes the struct into its binary form.
func (pkt *AuthAccountChangePassProofResp) Serialize(buf *protocol.Buffer) error {
	var start = buf.Size()
	if err := pkt.AuthAccountLogonProofResp.Serialize(buf); err != nil {
		return err
	}
	buf.WriteUInt8At(start+1, PidAuthAccountChangeProof)
	return nil
}
...
// Serialize encodes the struct into its binary form.
func (pkt *AuthAccountChangePassProofReq) Serialize(buf *protocol.Buffer) error {
	buf.WriteUInt8(ProtocolSig)
	buf.WriteUInt8(PidAuthAccountChangeProof)
	buf.WriteUInt16(88)
	buf.WriteBlob(pkt.ClientPasswordProof[:])
	buf.WriteBlob(pkt.NewSalt[:])
	buf.WriteBlob(pkt.NewVerifier[:])
	return nil
}

// Deserialize decodes the binary data generated by Serialize.
func (pkt *AuthAccountChangePassProofReq) Deserialize(buf *protocol.Buffer) error {
	if readPacketSize(buf) != 88 {
		return ErrInvalidPacketSize
	}
	copy(pkt.ClientPasswordProof[:], buf.ReadBlob(20))
	copy(pkt.NewSalt[:], buf.ReadBlob(32))
	copy(pkt.NewVerifier[:], buf.ReadBlob(32))
	return nil
}
